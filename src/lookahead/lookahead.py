...
from transformers.models.led.modeling_led import LEDEncoderBaseModelOutput
import dataclasses
...
class Lookahead:
    ...
    def score(
        ...
        **model_kwargs,
        ...
        # indices is used to keep track of the example id for each input id. Before beam search, it's just a range()
        ...
        # Expand both indices and inputs by lookahead_top_k
        # (1) indices: batch_size -> batch_size * lookahead_top_k
        indices = indices.repeat_interleave(self.lookahead_top_k)
        # (2) input_ids: batch_size -> batch_size * lookahead_top_k
    ...
    # Copied from generation_utils.greedy_search
    def greedy_search(
        self,
        ...
        **model_kwargs,
        ...
        # !NEW! - calculate length at which to break the generation
        ...
        while True:
            ...
            # !NEW!
            # Lookahead break - don't want for stopping criteria
    ...
    # Copied from generation_utils.sample
    def sample(
            self,
            input_ids: torch.LongTensor,
            beam_scorer=None,
    ...
    # Copied from generation_utils.beam_search
    def beam_search(
        self,
        input_ids: torch.LongTensor,
        beam_scorer = None,
    ...
    def expand_model_kwargs(self, model_kwargs, indices):
        ...
        # Hack support for LED because deepcopy of the output dataclass throws an error
        is_led_output = isinstance(model_kwargs['encoder_outputs'], LEDEncoderBaseModelOutput)
        if is_led_output:
            encoder_outputs = model_kwargs.pop('encoder_outputs')
            encoder_outputs_copy = {}
            for key, value in dataclasses.asdict(encoder_outputs).items():
                encoder_outputs_copy[key] = copy.deepcopy(value)
            encoder_outputs = LEDEncoderBaseModelOutput(**encoder_outputs_copy)
