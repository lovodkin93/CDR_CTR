...
class Generator(GenerationMixin):
    ...
    def eval(self):
        """
        NEW - necessary for quark policy
        """

        self.model.eval()

    # Copied from generation_utils.generate
    @torch.no_grad()
    def generate(
        self,
        ...
        **model_kwargs,
        ...
        ```"""
        # !NEW! - in a lot of places, self. was replaced with self.model.
    ...
    def greedy_search(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # pre-process distribution - !NEW! (moved from below to up here)
    ...
    def sample(
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # !NEW! - run warper only if not working with lookahead - call to warper was here before
    ...
    # Copied from generation_utils.beam_sample
    def beam_sample(
        self,
        input_ids: torch.LongTensor,
        beam_scorer: BeamScorer,
        ...
        **model_kwargs,
        ...
        while True:
            ...
            # !NEW! - run warper only if not working with lookahead (call to warper was here previously)
            if self.lookahead is None:
                next_token_scores = logits_warper(input_ids, next_token_scores)
        ...
        if return_dict_in_generate:
            if not output_scores:
                sequence_outputs["sequence_scores"] = None
            # \/ !PREV! - Deleted (seems unrelated to this project, just out of date)
            # else:
            #     num_return_sequences = beam_scorer.num_beam_hyps_to_keep
            #     # return only as many indices as sequences
            #     beam_indices = tuple(
